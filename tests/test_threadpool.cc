...
// Dummy ShouldEqualSequence for demonstration
template<typename It1, typename It2>
void ShouldEqualSequence(It1 begin1, It1 end1, It2 begin2) {
  for (; begin1 != end1; ++begin1, ++begin2) {ASSERT_EQ(*begin1, *begin2);}
}
...
TEST(ThreadPoolTest, ThreadPoolBasic) {
  ...
  for (size_t i = 0; i < v_expected.size(); ++i) {v_expected[i] = i*(i+1)/2;}
...
TEST(ThreadPoolTest, ThreadPoolException) {
    ...
    futures.emplace_back(
      pool.enqueue(
        [&v, &exception_string, i] (size_t /*thread_id*/) {
          v[i] = 1;
          if (i == 5000) {throw std::runtime_error(exception_string);}
        }
      )
    );
  }
  try {
    ...
    if (ex.what() == exception_string) {caught = true;}
  }
  ASSERT_TRUE(caught);
}
...
TEST(ThreadPoolTest, ParallelForEach) {
  ...
  parallel_foreach(4, v_in.begin(), v_in.end(),
    [&v_out] (size_t /*thread_id*/, int x) {v_out[x] = x*(x+1)/2;}
  );
  ...
  try {
    parallel_foreach(4, v_in.begin(), v_in.end(),
      [&v_out, &exception_string] (size_t /*thread_id*/, int x) {
        if (x == 5000) {throw std::runtime_error(exception_string);}
        v_out[x] = x;
      }
    );
  }
...
TEST(ThreadPoolTest, ParallelForEachSum) {
  ...
  parallel_foreach(n_threads, input.begin(), input.end(),
    [&results] (size_t thread_id, size_t x) {results[thread_id] += x;}
  );
...
TEST(ThreadPoolTest, DISABLED_ParallelForEachTiming) {
  ...
  std::vector<size_t> results(n_threads, 0);
  parallel_foreach(n_threads, input.begin(), input.end(),
    [&results] (size_t thread_id, size_t /*x*/) {results[thread_id] += 1;}
  );
